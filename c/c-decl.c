	     ...
	     tree origtype, tree asmspec_tree)
{
  ...
  if (decl->base.code == VAR_DECL && Wmisra_c_trigger && decl->typed.type->base.code == POINTER_TYPE) {
	...
	while (misra_18_5_pointer_check) {
			       ...
			       tree orig_name)
{
  ...
  if (type_mv != unsigned_type_node) {
	inform(loc, "==========================Misra-c 2012 rule violation:6.2==========================\n");	
  }
  if (type_mv == long_integer_type_node && type_mv != unsigned_type_node) {
	inform(loc, "\n==========================Misra-c 2012 rule violation:6.1==========================\nNon-compliant even if long and int are the same size\n");	
  }
